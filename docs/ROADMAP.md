...
- **Kills: a verified spine + a corroboration layer.** `PanelKillCounter`
  ...
  ~80% recall. `KillfeedSegmenter` (`killfeed_segmenter_classical@0.1.0`) separates
...
| **4** ✅ | Kills → `KillEvent`/`DeathEvent` (+ killfeed `WeaponEvent`/`TradeEvent`) | **done, fully measured**: `PanelKillCounter` exact vs the post-game card (8/8, 106/79) = the kill spine. All `245` killfeed rows visually labelled (2026-07-01): detector precision `0.449`, spine recall `0.3081` (53/172 panel kills identity-matched), `107` content rows / `83` unique kills / `12` headshots / `10` trades / 1 grenade. `killfeed_content_knn@0.2.0` (different-content margin confidence) measured LOO: **abstains at deployment threshold** (background-dominated embedding — gate holds); forced diagnostic attacker `0.36` / victim `0.39` / kill_type `0.98`. Labelled rows are the production enrichment path for this match; an OCR/icon model is the upgrade for new VODs. |
...
| **10+** | Tactical layer, elite metrics, comms metrics, autonomous orchestration | each: defined, evidenced, evaluated |
