"""Evaluate Phase 5 coarse kill-type recognition."""
...
def _expected(row: dict[str, Any]) -> str:
    return str(row["label"]["kill_type"])


def _score(rows: list[dict[str, Any]], predictions: list) -> dict[str, Any]:
    n = len(rows)
    expected = [_expected(row) for row in rows]
    predicted = [prediction.kill_type if prediction is not None else None for prediction in predictions]
    ...
    confidence_values = [p.confidence for p in predictions if p is not None and p.kill_type is not None]
...
def _gallery(dataset_dir: Path, rows: list[dict[str, Any]], approach: Approach) -> dict[str, Any]:
    recognizer = KillTypeRecognizer(dataset_dir, approach=approach, confidence_threshold=0.0)
    return {
...
def evaluate(dataset_dir: Path, approaches: list[Approach] | None = None) -> dict[str, Any]:
    approaches = approaches or ["template", "histogram"]
    manifest, rows = load_kill_type_rows(dataset_dir)
    labelled = labeled_kill_type_rows(rows)
    label_counts = Counter(row["label"].get("kill_type") for row in labelled)
    unclear = sum(1 for row in rows if row.get("label", {}).get("unclear") is True)
    unknown = sum(1 for row in rows if row.get("label", {}).get("kill_type") == "unknown")
    ...
    result: dict[str, Any] = {
        "dataset": {
            ...
            "icon_count": len(rows),
            "labelled_kill_type_icons": len(labelled),
            "label_counts": dict(label_counts),
            "unknown_labels": unknown,
            "unclear_labels": unclear,
            "categories": list(manifest.get("categories") or KILL_TYPE_CATEGORIES),
            "label_status": "labeled" if labelled else "unlabeled",
        },
        "models": {},
        "notes": [
            "Kill-type recognition is independent from player-name OCR.",
            "Downstream analytics consume kill_type, not exact weapon names.",
            "No real accuracy is reported until kill-type icons are human-labelled.",
            "Operational gallery is a regression check; leave-one-out is the honest small-data estimate.",
            "Low-confidence predictions return kill_type=null.",
        ],
    }
    ...
    for approach in approaches:
        ...
        if not labelled:
            result["models"][approach] = {
                "model": model,
                "status": "unlabeled - no kill-type accuracy claim",
...
def print_report(result: dict[str, Any]) -> None:
    d = result["dataset"]
    print("# Kill-Type Recognition Evaluation")
    print()
    print(f"- dataset: {d['path']}")
    print(f"- icons: {d['icon_count']} | labelled: {d['labelled_kill_type_icons']}")
